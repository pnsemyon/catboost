...
namespace NCatboostCuda {
    ...
    TMetricHolder TGpuCustomMetric::Eval(const TStripeBuffer<const float>& target,
        ...
        // CB_ENSURE(false, "Calling custom eval from gpu metric!\n");
        CATBOOST_INFO_LOG("Calling custom eval from gpu metric", target.At(0).Get(), weights.At(0).Get(), cursor.At(0).Get());
        return (*(Descriptor.EvalFunc))(cursor_ptr, target_ptr, weights_ptr, 0, 0, Descriptor.CustomData);
    }
    ...
    TMetricHolder TGpuCustomMetric::Eval(const TMirrorBuffer<const float>& target,
        ...
        auto cursor_ptr = TConstArrayRef<float>(cursor.At(0).Get(), cursor.At(0).ObjectCount());
        // CB_ENSURE(false, "Calling custom eval from gpu metric!\n");
        CATBOOST_INFO_LOG("Calling custom eval from gpu metric mirror", target.At(0).Get(), weights.At(0).Get(), cursor.At(0).Get());
        return (*(Descriptor.EvalFunc))(cursor_ptr, target_ptr, weights_ptr, 0, 0, Descriptor.CustomData);
    }
